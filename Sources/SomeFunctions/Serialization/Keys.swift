...
private class CC {
  ...
  class RSA {
    ...
    static func encrypt(_ data: Data, derKey: Data, tag: Data, padding: AsymmetricPadding,
      ...
      let status = buffer.withUnsafeMutableBytes { buffer in
        data.withUnsafeBytes { data in
          tag.withUnsafeBytes { tag in
            CCRSACryptorEncrypt!(
              key, padding.rawValue,
              data.baseAddress!, data.count,
              buffer.baseAddress!, &bufferSize,
              tag.baseAddress!, tag.count,
              digest.rawValue)
          }
        }
      }
    ...
    static func decrypt(_ data: Data, derKey: Data, tag: Data, padding: AsymmetricPadding,
      ...
      let status: CCCryptorStatus = buffer.withUnsafeMutableBytes { buffer in
        data.withUnsafeBytes { data in
          tag.withUnsafeBytes { tag in
            CCRSACryptorDecrypt!(
              key, padding.rawValue,
              data.baseAddress!, bufferSize,
              buffer.baseAddress!, &bufferSize,
              tag.baseAddress!, tag.count,
              digest.rawValue)
          }
    ...
    private static func importFromDERKey(_ derKey: Data) throws -> CCRSACryptorRef {
      ...
      let status = derKey.withUnsafeBytes {
        CCRSACryptorImport!($0.baseAddress!, derKey.count, &key)
      }
      try check(status: status)
    ...
    private static func exportToDERKey(_ key: CCRSACryptorRef) throws -> Data {
      ...
      let status = derKey.withUnsafeMutableBytes {
        CCRSACryptorExport!(key, $0.baseAddress!, &derKeyLength)
      }
      try check(status: status)
